...
var countRangeSum = function (nums, lower, upper) {
  ...
  const merge_sort = (lo, hi, lower, upper) => {
    ...
    let count =
      merge_sort(lo, mid, lower, upper) + merge_sort(mid, hi, lower, upper);
    let right1 = mid,
      right2 = mid;

    for (let left = lo; left < mid; left++) {
      while (right1 != hi && raw_nums[right1] - raw_nums[left] < lower)
        right1++;
      while (right2 != hi && raw_nums[right2] - raw_nums[left] <= upper)
        right2++;
