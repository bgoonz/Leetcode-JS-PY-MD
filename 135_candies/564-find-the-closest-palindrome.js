...
function getPrevPalindrome(number) {
  ...
  const comparison = compareTwoValues(
    BigInt(palindromedLeft),
    BigInt(rightSide)
  );
  if (comparison === 0) {
  ...
  if (comparison === 1) {
    ...
    const newWhole = BigInt(
      decremented.toString() + getRightSideNumberAsString(decrementedNumber)
    );
...
function getNextPalindrome(number) {
  ...
  if (comparison === -1) {
    ...
    const newWhole = BigInt(
      incrementedLeftWithBorder.toString() +
        getRightSideNumberAsString(incrementedNumber)
    );

    const newLeft = getLeftSideNumber(newWhole);
    const palindromedNewLeft = getPalindromeAsString(newLeft);
    return BigInt(
      incrementedLeftWithBorder.toString() + palindromedNewLeft.toString()
    );
  }
}
